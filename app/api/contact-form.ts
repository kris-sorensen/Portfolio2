...
export default async function handler(
  ...
  if (upsertTimelineEntryRes.error) {
    ...
    return res
      .status(500)
      .json({ error: upsertTimelineEntryRes.error.message });
  }

  console.log(
    `Custom timeline entry upserted ${upsertTimelineEntryRes.data.timelineEntry.id}.`
  );
